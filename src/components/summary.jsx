...
export class Summary extends React.Component {
	...
	componentWillMount() {
		...
		if (userData && userData.wallet) {
			this.setState({ assets: userData.wallet });
		}
	}

	componentWillReceiveProps(nextProps) {
		const { userData } = this.props;
		if (JSON.stringify(userData) !== JSON.stringify(nextProps.userData)) {
			if (nextProps.userData && nextProps.userData.wallet) {
				this.setState({ assets: nextProps.userData.wallet });
			} else {
				this.setState({ ssets: { USD: 0 } });
			}
		}
	}
			...
			(err, res, body) => {
				if (err) {
					...
					if (res.statusCode === 409) {
						...
						toast(
							`${amount < 0 ? "Sold" : "Bought"} ${body.amount} ${symbol}`,
							3000
						);
			...
			<table className="highlight responsive-table">
				<thead>
					<tr>
						...
						<th>Value</th>
						<th>
							<span className="hide-on-large-only">Trading</span>
							<span className="hide-on-med-and-down">
								You have: ${this.state.assets.USD.toPrecision(7)}
							</span>
						</th>
					</tr>
					<p className="hide-on-large-only">
						You have:<br />${this.state.assets.USD.toPrecision(7)}
					</p>
				</thead>
						...
						<tr key={symbol}>
							...
							<td>
								{this.props.priceData
									? this.props.priceData[symbol].USD
									: "N/A"}
							</td>
							...
							<td className="flex-row">
								...
								<button
									...
									onClick={() => {
										if (parseFloat(this.refs[symbol].value) > 0) {
											this.transaction(
												symbol,
												parseFloat(this.refs[symbol].value)
											);
										}
									}}
								>
								...
								<button
									className="waves-effect waves-teal btn-flat hide-on-small-only"
									onClick={() => {
